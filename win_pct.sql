...
--Find total wins, losses, and games played per surface
total_wins AS
(select 
    winner_name,
    surface,
...
(select 
    ...
    coalesce(total_wins.surface, total_losses.surface) as surface,
    coalesce(wins, 0) as wins,
    coalesce(wins, 0) + coalesce(losses, 0) as total_played
from total_wins
full outer join total_losses
on total_wins.winner_name=total_losses.loser_name and
total_wins.surface=total_losses.surface
order by name)

--Find the win percentage per surface
select 
    name,
    total_games.surface,
    total_played,
